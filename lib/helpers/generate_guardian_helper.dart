...
class GenerateGuardianHelper {
  ...
  List<String> _getRandomGuards(List<Employee> guards) {
    ...
    guards.forEach((element) {
      if (_tempGuards.length > 0) {
        Employee _selectedGuard = _tempGuards[_getRandomIndex(_tempGuards.length)];
        numbers.add(_selectedGuard.phoneNumber);
        _tempGuards.removeWhere((element) =>
            element.team == _selectedGuard.team && element.role == _selectedGuard.role);
      }
    });
  ...
  List<Map<String, List<String>>> generateGuardianDateList(List<Employee> employeeList,
      [DateTime start]) {
    List<Map<String, List<String>>> _employeesWithDate = [];
    DateTime startDate = start ?? DateTime.now();
    DateTime lastDate = DateTime(2020, 08, 15);
    ...
    dates.forEach((element) {
      ...
      List<Employee> guards = employeeList.where((element) => element.isGuard).toList();
          ...
          employeWithDateItem.forEach((key, value) {
            if (key == lastDateTime) {
              lastDevelopers.forEach((element) {
                String phoneNumber =
                    value.firstWhere((item) => item == element, orElse: () => null);
                if (phoneNumber != null) {
